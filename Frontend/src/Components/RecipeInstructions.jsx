    ...
    return(
        <div className={styles.RecipeInstructionsContainer}>
            { 
                ...
                ? <ul key={index} className={styles[content.type]}>{content.children.map(content2 => content2.children.map((content3, index2) => <li key={index2} className={`${content3.code ? styles.code : ""} ${content3.italic ? styles.italic : ""} ${content3.underline ? styles.underline : ""} ${content3.bold ? styles.bold : ""}`}>{content3.text}</li>) )}</ul>
                : (content.type === "image") 
