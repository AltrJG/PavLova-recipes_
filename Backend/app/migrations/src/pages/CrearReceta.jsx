...
import RecipeNutritionalFacts from '../Components/RecipeNutritionalFacts';
...
export default function CrearReceta() {
  ...
  useEffect(() => {
    ...
    const obtenerRecetaActualizar = async () => {
      try{
        ...
        const ingredientesUpdate = recetaActualizar.data.ingredientes.map(item => {
            return {
                value: item.ingrediente.nombre,
                label: item.ingrediente.nombre,
                image: item.ingrediente.foto_ingrediente,
                consistencia: item.ingrediente.consistencia,
        ...
        setIsUpdateActive(recetaActualizar.data.porciones);
      } catch(error){
  ...
  const handleIngredientesSeleccionados = ingredientesActivos => {
    let newIngredients = ingredientesActivos.map(item => {
      if(activeIngredientOptions.findIndex(ingred => ingred.id == item.id) == -1){
        return{
          value: item.value,
          label: item.label,
          image: item.image,
...
const subirImagenReceta = async (recetaId) => {
  ...
  const options = [
    { type: 'Ingredientes', icon: 'nutrition', label: 'Ingredientes' },
    { type: 'Informacion', icon: 'information-circle', label: 'Info. nutricional' },
  ];
    ...
    useEffect(() => {
      let nutrientes = calcularNutrientes(activeIngredientOptions, porciones, porciones);
      setNutritionalValues(nutrientes);
    }, [activeIngredientOptions, porciones]);
      ...
      <div className={styles.crearRecetaOptions}>
        ...
        <div className={styles.formContainer}>
          ...
          <div className={`${styles.section} ${active === "ingre" ? styles.activeSection : ""}`}>
            <div className={styles.formInfoSelect}>        
              ...
              <div className={styles.formOptions}>
                {options.map(option => (
                  <OptionButton
                    ...
                    icon={option.icon} 
                    makeRowOnMobile={true} 
                  />
                ))}  
              </div>
            </div>
            <div className={`${styles.nutritionalInfo} ${activeOption == 'Informacion' ? styles.activeTab : ''}`}>
              <RecipeNutritionalFacts/>
            </div>
            <div className={`${styles.ingredientSelector} ${activeOption == 'Ingredientes' ? styles.activeTab : ''}`}>
