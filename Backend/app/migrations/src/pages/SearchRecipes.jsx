...
export default function SearchRecipes(){
    ...
    const { refreshAccessToken, isSuperUser, isStaff, isLoading } = useAuth();
    ...
    const getRecipes = async (previous = null, next = null, noFilters = false, name = null) => {
        ...
        try{
            ...
            next == null && setRecipes(response.data.results);
            next != null && setRecipes(prevRecipes => [...prevRecipes, ...response.data.results]);
        } catch(error){
            if(error.response?.status == 401){
    ...
    const deleteFilters = async () => {
        ...
        setActiveCategoria('');
        resetRecipeFilters();
        await getRecipes(null, null, true, null, true);
    }
    ...
    useEffect(() => {
        ...
        if(isLoading && (isStaff || isSuperUser)){
            triggerUpdateRecipes();
        }
    }, [isLoading]);
