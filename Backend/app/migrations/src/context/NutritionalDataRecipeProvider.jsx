...
const NutritionalDataRecipeProvider = ({ children }) => {
    ...
    const changePortion = porcionesData => {
        ...
        let newIngredientValues = ingredientes.map(ingredient => {
            ...
            return {
                text: `${conversion.toFixed(2)} ${ingredient.unidad == 'numerica' ? (ingredient.ingrediente.consistencia == 'solido' ? "g" : "ml") : ingredient.unidad == 'cucharadita' ? "cdta." : (ingredient.unidad == "cucharada" ? "cda." : (ingredient.unidad == "taza" ? "taza" : ""))} ${['taza', 'cucharadita', 'cucharada', 'numerica'].includes(ingredient.unidad) ? 'de' : ''} ${!['taza', 'cucharadita', 'cucharada', 'numerica'].includes(ingredient.unidad) ? ingredient.unidad.trim().split(' ').slice(1).join(' ') : ingredient.ingrediente.nombre} ${(isSuperUser || isStaff) ? `(EA: ${ingredient.ingrediente.escala_agua})` : ""}`,
